...
export default function DashboardPage() {
  ...
  const handleCreateSampleData = async () => {
    ...
    try {
      ...
      // Refrescar los datos después de crear los datos de prueba
      await Promise.all([
        refetchFarms(),
        refetchCrops(), 
        refetchTasks()
  ...
  const totalArea = farms?.reduce((sum, farm) => sum + (farm.size || 0), 0) || 0;
          ...
          <div className="flex items-center space-x-3 mb-2">
            ...
            <div>
              ...
              <p className="text-slate-600 text-lg">
                Bienvenido, {currentUser?.displayName?.split(' ')[0] || currentUser?.email?.split('@')[0] || 'Usuario'} 👨‍🌾
              </p>
            </div>
          </div>
          <p className="text-slate-500 max-w-2xl">
            Monitorea y gestiona todas las actividades de tu operación agrícola desde un solo lugar. 
            Mantente al día con tus cultivos, tareas y productividad.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="space-y-4">
            {tasks?.slice(0, 5).map((task) => (
              <div key={task.id} className="flex items-start space-x-4 p-4 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid gap-4">
            <Link
              to="/farms"
              className="group flex items-center p-4 bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-200 rounded-xl hover:from-blue-100 hover:to-blue-200 transition-all duration-200"
            >
              <div className="w-10 h-10 bg-blue-500 rounded-lg flex items-center justify-center mr-4">
                <MapPin className="w-5 h-5 text-white" />
              </div>
              <div className="flex-1">
                <p className="font-semibold text-blue-900">Nueva Finca</p>
                <p className="text-sm text-blue-700">Registra una nueva propiedad</p>
              </div>
            ...
            <Link
              to="/crops"
              className="group flex items-center p-4 bg-gradient-to-r from-emerald-50 to-emerald-100 border border-emerald-200 rounded-xl hover:from-emerald-100 hover:to-emerald-200 transition-all duration-200"
            >
              <div className="w-10 h-10 bg-emerald-500 rounded-lg flex items-center justify-center mr-4">
                <Wheat className="w-5 h-5 text-white" />
              </div>
              <div className="flex-1">
                <p className="font-semibold text-emerald-900">Nuevo Cultivo</p>
                <p className="text-sm text-emerald-700">Registra una nueva plantación</p>
              </div>
            ...
            <Link
              to="/tasks"
              className="group flex items-center p-4 bg-gradient-to-r from-amber-50 to-amber-100 border border-amber-200 rounded-xl hover:from-amber-100 hover:to-amber-200 transition-all duration-200"
            >
              <div className="w-10 h-10 bg-amber-500 rounded-lg flex items-center justify-center mr-4">
                <CheckSquare className="w-5 h-5 text-white" />
              </div>
              <div className="flex-1">
                <p className="font-semibold text-amber-900">Nueva Tarea</p>
                <p className="text-sm text-amber-700">Programa una actividad</p>
              </div>
