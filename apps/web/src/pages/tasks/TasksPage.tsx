...
import { formatDateTime, TASK_TYPES, TASK_PRIORITIES, TaskStatus } from '@campo360/lib';
import type { Task, CreateTaskDto } from '@campo360/lib';

const taskSchema = z.object({
  title: z.string().min(3, 'El título debe tener al menos 3 caracteres'),
  description: z.string().optional(),
  type: z.string().min(1, 'Debes seleccionar un tipo de tarea'),
  priority: z.string().min(1, 'Debes seleccionar una prioridad'),
  scheduledDate: z.string().min(1, 'La fecha de vencimiento es requerida'),
...
export default function TasksPage() {
  ...
  // Mapeo de valores de español (frontend) a inglés (backend)
  const taskTypeMap: { [key: string]: string } = {
    ...
    'Cosecha': 'HARVESTING',
    'Preparación del suelo': 'SOIL_PREPARATION',
  ...
  const onSubmit = async (data: TaskForm) => {
    ...
    try {
      // Convertir la fecha al formato ISO-8601 completo
      const scheduledDate = new Date(data.scheduledDate).toISOString();
      
      const mappedData: CreateTaskDto = {
        ...data,
        type: taskTypeMap[data.type] as any,
        priority: priorityMap[data.priority] as any,
        farmId: data.farmId || '',
        cropId: data.cropId || undefined,
        scheduledDate: scheduledDate,
      };
      
      await tasksService.createTask(mappedData);
      ...
      toast.error('Error al crear tarea', error.response?.data?.message || 'Ha ocurrido un error');
    } finally {
